#!/usr/bin/env python3
"""
RootTOP Assistant - Standalone EXE Version
Everything included, no external dependencies needed after build
"""
...
# Fix for PyInstaller - get correct path
...
try:
    ...
    import subprocess
    subprocess.check_call([sys.executable, "-m", "pip", "install", "llama-cpp-python==0.2.83", "--quiet"])
    import llama_cpp
...
2. NEVER respond in Vietnamese, Chinese, Japanese, Korean
...
class RootTOPAssistant:
    ...
    def create_header(self):
        ...
        if os.path.exists(logo_path) and PIL_AVAILABLE:
            try:
                ...
                logo_label = tk.Label(logo_frame, image=self.logo_img, bg='#0f3460')
    ...
    def create_default_logo(self, parent):
        logo_bg = tk.Label(parent, text="™", font=('Segoe UI', 32, 'bold'),
                           fg='#FFD700', bg='#0f3460')
    ...
    def create_status_bar(self):
        ...
        copyright_label = tk.Label(status_bar, text=f"© 2024 {COMPANY_NAME} | Right-click on messages to copy",
    ...
    def create_input_panel(self):
        ...
        tk.Label(input_frame, text="💬 Your message (English / বাংলা) - Right-click to copy:", 
...
• Click Download Model (first time only)
...
© 2024 {COMPANY_NAME}"""
    ...
    def download_model(self):
        ...
        confirm = messagebox.askyesno("Download Model", 
            f"Download Model ({MODEL_CONFIG['size']})?\n\n"
    ...
    def _download_thread(self):
        try:
            target_path = self.models_dir / MODEL_CONFIG["filename"]
            ...
            urllib.request.urlretrieve(MODEL_CONFIG["url"], str(target_path), reporthook=report)
    ...
    def load_model(self):
        ...
        def load():
            try:
                ...
                self.add_message(ASSISTANT_NAME, "✅ Ready! Right-click on any message to copy text.", is_user=False)
